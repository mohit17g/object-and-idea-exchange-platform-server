...
// const { mapPostOutput1 } = require("../utils/Utils1");
...
const getPostsproductsOfAll = async (req, res) => {
  try {
    const allPostsproducts = await Postproduct.find().populate("owner");
    const postsproducts = allPostsproducts
      .map((item) => mapPostOutput(item, req._id))
      .reverse();
    return res.send(success(200, { postsproducts }));
  } catch (error) {
    console.log(e);
    return res.send(error(500, e.message));
  }
